...
def test(xyz, target, labels, save_path, focus_ratio, iter = None, lock = None):
    ...
    #    n_true_clusters = 10
    print("Watershed found {} clusters".format(len(np.unique(labels))))
    ...
    if iter is not None:
        ...
        visualize(xyz, labels, target, save_path=save_path + f'-{iter+1}.png', s=10, a = 0.5)
...
if __name__ == '__main__':
    ...
    for i in range(1, iter + 1):
        # load las files
        las_path = os.path.join('./results/', dataset, subfolder + '-'+ str(i) + '-segmented.laz')
        if os.path.exists(las_path):
            ...
            break
    # with threading
    save_path = os.path.join('./results/', dataset, subfolder + '-watershed')
    threads = []
    lock = threading.Lock()
