...
from architecture.SGPN import SGPN
...
def visualize_proper(points, labels, targets, figsize = (30,30), s = 100, save_path = None, a = 1):
...
def calc_iou(labels, targets):
    ...
    print("Calculating IoU with new method")
    ious = np.array([[0.0] * len(np.unique(labels))] * len(np.unique(targets)))
    unique_targets = np.unique(targets)
    unique_labels = np.unique(labels)
    for T, t in enumerate(unique_targets):
        for L, l in enumerate(unique_labels):
            intersection = np.sum((targets == t) & (labels == l))
            union = np.sum((targets == t) | (labels == l))
            iou = intersection / union
            ious[T, L] = iou
...
def test(xyz, target, features, save_path, method, focus_ratio, iter = None, lock = None):
    ...
    #    n_true_clusters = 10
    print(features.shape)
    labels = cluster(features, n_clusters=n_true_clusters, save_path=None, target=target, method = method)
    ...
    if iter is not None:
        ...
        # acquire lock
        lock.acquire()
        visualize_proper(xyz, labels, target, save_path=os.path.join(save_path,f'{iter}.png'), s=69, a = 0.5)
        lock.release()
        results[iter] = (miou, AP, AP_50, AP_25)
    else:
        visualize(xyz, labels, save_path=os.path.join(save_path,'pred.png'))
...
if __name__ == '__main__':
    ...
    save_path = os.path.join('./results', model_basename, dataset + '-' + subfolder)
    ...
    batch_size = 8
    dataloader = get_dataloader(data_dir, subfolder, batch_size, 1, max_trees = 50, point_count = 4096, sampling='random', preload=False, center=True, normalize=True, select_trees = None, annotated = True, aug_rot = False, shuffle = False)
    ...
    for i, (xyz, target, features) in enumerate(zip(xyzs, targets, featuress)):
        ...
        t = threading.Thread(target=test, args=(xyz, target, features, save_path, method, focus_ratio, i), kwargs={'lock': lock})
        threads.append(t)
        t.start()
        #test(xyz, target, features, save_path, method, focus_ratio, i, lock)
