...
def test(xyz, target, labels, save_path, focus_ratio, iter = None, lock = None):
    ...
    if len(xyz) > 40000:
        try:
            ...
            print("Not enough points to visualize: ", len(xyz))
            exit(1)
        xyz = xyz[idx]
        labels = labels[idx]
        target = target[idx]
    if iter is not None:
        ...
        # select subset of points for visualization
        
        
        visualize(xyz, labels, target, save_path=save_path + f'-{iter+1}.png', s=69, a = 0.5)
...
if __name__ == '__main__':
    ...
    for i in range(1, iter + 1):
        # load las files
        las_path = os.path.join('./results/', dataset, subfolder + '-'+ str(i) + '-ams3d.laz')
    ...
    # with threading
    save_path = os.path.join('./results/', dataset, subfolder + '-ams3d')
    threads = []
    lock = threading.Lock()
    ...
    # print mean of mIoU, AP, AP@50, AP@25 with standard deviation in brackets all in one line and put & between them
    print("AMS3D & ", end='')
    for i in range(4):
